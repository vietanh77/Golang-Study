...
func NewUnauthorized(rootError error, msg, log, key string) *AppError {
	return &AppError{
...
func ErrorInterial(err error) *AppError {
	return NewFullErrorResponse(http.StatusInternalServerError, err,
		"something went wrong in server", err.Error(), "ErrorInterial")
}

func CannotListEmpty(entity string, err error) *AppError {
...
func ErrEntityDeleted(entity string, err error) *AppError {
	return NewCustomError(
		...
		fmt.Sprintf("Err%sDeteled", entity),
	)
}
...
func ErrorNoPermisison(err error) *AppError {
	return NewCustomError(
		err,
		fmt.Sprintf("You have no permission"),
		fmt.Sprintf("ErrNoPerrmission"),
	)
}
